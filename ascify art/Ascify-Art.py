...
if sys.platform.startswith("win"):
    ...
    try:
        ...
        windll.dwmapi.DwmSetWindowAttribute(
            HWND, 35, byref(c_int(0x34241E)), sizeof(c_int)
        )
    except:
        pass
...
frame_image = customtkinter.CTkFrame(
    root, width=350, fg_color="#1b202c", corner_radius=20
)
...
label_image = customtkinter.CTkLabel(
    frame_image, width=350, fg_color="#1b202c", corner_radius=0, text=""
)
...
def operation():
    ...
    outputImage = Image.new(
        "RGB", (oneCharWidth * width, oneCharHeight * height), color=background
    )
    ...
    image = customtkinter.CTkImage(
        outputImage,
        size=(
            frame_image.winfo_height(),
            frame_image.winfo_height() * img.size[1] / img.size[0],
        ),
    )
...
def open_sequence():
    ...
    if dir_img := filedialog.askdirectory():
        ...
        RightClickMenu.add_command(
            label="Previous Frame", command=lambda: frame_previous()
        )
...
open_button = customtkinter.CTkButton(
    root, text="OPEN", fg_color=app_color, command=openfile
)
...
RightClickMenu2.add_command(
    label="Open Image Sequence", command=lambda: open_sequence()
)
...
text_live = customtkinter.CTkCheckBox(
    tabview.tab("Characters"),
    fg_color=app_color,
    text="Live Preview",
    hover=False,
    command=lambda: operation() if text_live.get() == 1 else None,
)
text_live.grid(row=0, column=0, padx=20, pady=10, sticky="ne")
text_live.select()

textbox = customtkinter.CTkTextbox(tabview.tab("Characters"), fg_color="#282c35")
...
textbox.insert(
    tk.END, "$@B%8&WM#*oahkbdpqwmZO0QLCJUYXzcvunxrjft/\\|()1{}[]?-_+~<>i!lI;:,\"^`'. "
)
textbox.bind(
    "<KeyRelease>", lambda event: operation() if text_live.get() == 1 else None
)
...
slider_scale = customtkinter.CTkSlider(
    ...
    from_=0.01,
    to=0.2,
    command=lambda e: label_14.configure(text=f"Scale factor: {str(round(e, 3))}"),
)
...
automatic = customtkinter.CTkSwitch(
    tabview.tab("Colors"),
...
slider_r = customtkinter.CTkSlider(
    tabview.tab("Colors"),
    height=20,
    progress_color="red",
    hover=False,
...
slider_b = customtkinter.CTkSlider(
    tabview.tab("Colors"),
    height=20,
    progress_color="blue",
    hover=False,
...
slider_g = customtkinter.CTkSlider(
    tabview.tab("Colors"),
    height=20,
...
def change_bg():
    ...
    pick_color = CTkColorPicker.AskColor(
        fg_color="#1e2434", bg_color="#1b202c", button_color=app_color
    )
...
bg_color = customtkinter.CTkButton(
    tabview.tab("Colors"),
    corner_radius=20,
    fg_color="black",
    border_width=2,
...
slider_sat = customtkinter.CTkSlider(
    tabview.tab("Colors"),
    height=20,
    width=10,
    hover=False,
...
slider_br = customtkinter.CTkSlider(
    tabview.tab("Colors"),
    height=20,
    width=10,
    hover=False,
        ...
        if "Emoji" not in filename and "18030" not in filename:
            ...
            try:
                customtkinter.CTkButton(
                    ...
                    text_color="white",
                    anchor="w",
                    command=lambda event=filename: change_font(event),
                ).grid(sticky="w")
...
load_more = customtkinter.CTkButton(
    tabview.tab("Font"),
    fg_color="#1b2f30",
    text="Load More",
    hover=False,
    command=add_more_fonts,
)
load_more.grid(row=1, sticky="sew", columnspan=2)

RightClickMenu3 = tk.Menu(
    frame_image,
    tearoff=False,
    background="#343e5a",
    fg="white",
...
var = tk.StringVar()
entry_save = customtkinter.CTkEntry(
    tabview.tab("Export"), textvariable=var, corner_radius=20, width=10
)
entry_save.grid(row=1, column=0, sticky="ew", padx=(20, 100))
entry_save._entry.configure(selectbackground=app_color)

format_ = customtkinter.CTkComboBox(
    tabview.tab("Export"),
    values=["png", "jpg", "bmp"],
    width=75,
    corner_radius=20,
...
def export():
    ...
    if sequence is False:
        ...
        if os.path.exists(new_dir):
            messagebox.showinfo(
                "Warning!",
                "A folder with this name already exists, please try a new name!",
            )
        else:
...
def new_window():
    ...
    label_top = customtkinter.CTkLabel(
        top_level, fg_color="#1b202c", text="Ascify-Art v0.8", font=("Roboto", 15)
    )
    label_top.grid(padx=20, pady=20, sticky="w")

    desc = "Developed by Akash Bora (Akascape) \n \nLicense: MIT \nCopyright 2023"
    label_disc = customtkinter.CTkLabel(
        top_level, fg_color="#1b202c", text=desc, justify="left", font=("Roboto", 12)
    )
    label_disc.grid(padx=20, pady=0, sticky="w")

    label_logo = customtkinter.CTkLabel(
        top_level, text="", image=logo, bg_color="#1b202c"
    )
    ...
    link.bind(
        "<Enter>",
        lambda event: link.configure(font=("", 13, "underline"), cursor="hand2"),
    )
    link.bind("<Leave>", lambda event: link.configure(font=("", 13), cursor="arrow"))

    link2 = customtkinter.CTkLabel(
        top_level,
        fg_color="#1b202c",
        text="Documentation",
        justify="left",
    ...
    link2.bind(
        "<Button-1>", lambda event: web("https://github.com/Akascape/Ascify-Art/wiki")
    )
    link2.bind(
        "<Enter>",
        lambda event: link2.configure(font=("", 13, "underline"), cursor="hand2"),
    )
...
entry_location = customtkinter.CTkButton(
    tabview.tab("Export"),
    corner_radius=20,
    width=10,
    fg_color="#343638",
...
progress_bar = customtkinter.CTkProgressBar(
    tabview.tab("Export"), height=20, width=10, progress_color=app_color
)
progress_bar.set(0)
...
save = customtkinter.CTkButton(
    ...
    text="SAVE",
    hover=False,
    command=lambda: threading.Thread(target=export).start(),
)
...
info.bind("<Button-1>", lambda event: new_window())
info.bind(
    "<Enter>", lambda event: info.configure(font=("", 13, "underline"), cursor="hand2")
)
