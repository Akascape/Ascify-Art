...
app_color = random.choice(["#487a7d","#e49c04","#84b701"])
...
textbox.bind("<KeyRelease>", lambda event: operation())

label_4 = customtkinter.CTkLabel(tabview.tab("Characters"), text="size: 15")
label_4.grid(row=0, column=0, sticky="en", pady=0, padx=30)

def update_label(value):
    label_4.configure(text="size: "+str(int(value)))

slider_size = customtkinter.CTkSlider(tabview.tab("Characters"), hover=False, width=10, button_color="white",
                                      from_=1, to=100, command=update_label)
slider_size.bind("<ButtonRelease-1>", lambda event: operation())
slider_size.grid(row=0, column=0, sticky="wen", pady=20, padx=(200,20))
slider_size.set(15)
...
format_ = customtkinter.CTkComboBox(tabview.tab("Export"), values=["png", "jpg", "bmp"], width=75, corner_radius=20, state="readonly")
    ...
    if sequence is False:
        ...
        if os.path.exists(exported_file):
            ...
            if res1=='yes':
                ...
                save.configure(state="Normal", fg_color=app_color, cursor="hand2")
                return
        else:
            outputImage.save(exported_file)            
